...
function GalleryItemForm(props){
    ...
    if (filepath){
        if (type === "picture") galleryItemDisplay = <img width="100%" src={__dirname + filepath}/>
        else if (type === "video"){
            galleryItemDisplay = (
                <video src={__dirname + filepath} width="320" height="240" controls>
                    <source src={__dirname + filepath} type={'video/'+filepath.split('.')[filepath.split('.').length - 1]}></source>
                </video>
            )
        }
    }
