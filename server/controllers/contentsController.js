...
exports.getContents = (req, res) => {
    var sql = "SELECT * FROM contents ORDER BY content_id DESC"
    var params = []
...
exports.createContents = (req, res) => {
  ...
  const { page_id, type, value } = req.body
  var sql = 'INSERT INTO contents (page_id, type, value ) VALUES (?,?,?)'
  var params = [ page_id, type, value ]
...
exports.updateContents = (req, res) => {
    ...
    db.run(
      `UPDATE contents SET 
          ...
          WHERE content_id = ?`,
      [ page_id, type, value, req.params.id],
      function (err, result) {
          console.log(err);
          if (err){
